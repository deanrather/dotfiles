# ~/.bashrc: executed by bash(1) for non-login shells (whenever opening a new terminal)
...
# Start Sublime if not running
if ! pgrep subl > /dev/null
then
  echo "Starting Sublime over X11-forwarding"
  subl &
fi
